...
The three analyzers landed (`node-vibrato`, `node-onset`, `node-breath`) — each realtime-safe (`process()` does no allocation; asserted) and paired with a CSV/grid sweep run in CI. Two of the four 1.2 follow-ups also closed (Tracer encoding removed; world schema is now registry-generated and rejects unknown node types and params at the schema layer). Two follow-ups remain — see below.
