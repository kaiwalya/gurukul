...
**Current phase: 1.5 — next up.** Phases 1.0–1.4 complete. This line is the single source of truth for project status; don't duplicate it elsewhere.
...
**Phase 1.5 — Visualisation layer.** The thing that makes Stage 1 "tuner-*plus*" rather than "tuner": a scrolling, multi-lane time graph filling the InGame surface beside the dial, so the singer sees the *shape* of a phrase, not just the current instant. Lanes: a tall **pitch trace** (absolute log2, y-window auto-zoomed to the last ~6 s of voiced range, scale-degree gridlines from `MusicInfo`, brightness = confidence, thickened/tinted where stable vibrato is detected), and a short **events lane** carrying onset ticks and breath spans. Pure presentation over the feature stream already flowing through `latest_features()` — no port or DSP change; the head ring-buffers `FeatureSnapshot` history and a dumb `time_graph` widget renders it, mirroring the `note_dial` split. This is what earns the "Stage 1 shippable" designation on Mac.
...
**Phase 1.6 — Phone port.** Rust core recompiled for `aarch64-apple-ios` and `aarch64-linux-android`. The plan of record: the same Bevy head cross-compiles to both — one codebase, different build config, Bevy rendering the same UI everywhere (no per-platform UI rewrite). Mic session plumbing is per platform (the audio-capture adapter, behind its port). The engine and analyzers do not change. The hexagonal split is the escape hatch: if Bevy fights a target (input, audio session, app-store constraints), only that platform's head is swapped for a native one (SwiftUI / Compose) — the `AppCoach` port and all business logic are reused untouched. This is where provisioning, device testing, and mobile audio-session quirks become live — deliberately deferred until there is something worth shipping.
