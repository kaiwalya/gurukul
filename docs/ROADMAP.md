...
**Phase 1.4 — Minimal live surface: Mac app. ✓ done.** First pixel drawn. SwiftUI app, CoreAudio HAL mic capture, the four analyzers running live, visualiser surface (big-note + cents readout, pitch trace, onset ticks, breath strip, vibrato readout). Practice-mode world running against a real mic. ECS was originally scoped for this phase as "visualiser entities with `PortBinding(path)` components" — slipped to Phase 1.5 at the start of PR 1.4.7 (decision recorded in `docs/PHASE_1_4.md` §7). Rationale: four fixed signals + four hard-wired views does not justify the abstraction; the cost of ECS now is greater than the cost of refactoring four views into entities later, if it ever becomes warranted. **Phase 1.4.8** added audio-I/O preferences (input/output device + engine sample-rate pickers with engine rebuild) and an interactive debug pane (any node + port, with monitor route for audio-shaped ports). See [`PHASE_1_4_8.md`](PHASE_1_4_8.md) for the full sub-phase plan.
